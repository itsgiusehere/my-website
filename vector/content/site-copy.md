# Site Copy — Giuseppe de Cesare
# Status: First draft — awaiting Giuseppe's visual review
# Last updated: 2026-03-05

---

## Section 1: Hero

**Name (small, above headline):**
Giuseppe de Cesare

**Headline:**
I turn fragmented thinking into shared clarity — so your teams can move forward.

**Subheadline:**
Design leaders and CPOs bring me in when everyone has a different picture of where things are going. I synthesise across product, business, and user needs — and deliver something concrete enough to align around and build from.

**Primary CTA:** Get in touch

---

## Section 2: The Problem

**Headline:**
You know what needs to happen. You can't get everyone to see it the same way yet.

**Body:**
Product, design, business — everyone has a different read on the priority, the problem, or the direction. The initiative has been "almost ready to move" for months. There's no shortage of opinions. What's missing is something concrete enough for everyone to commit to.

Strategy documents don't unlock that. Pixel-perfect designs don't either. What moves things is a tangible vision — something you can point to, test, and build from.

---

## Section 3: How I Work

**Section label (small):** How I work

**Left anchor statement — reacts to open card:**
- Vision Sprint open: "I speak with users, PMs, marketers, engineers — and design the concepts that bring them together."
- Embedded Design Leadership open: "I embed with your team and drive from vision through delivery — so you're not chasing progress."
- Strategic Clarity open: "I design the process that gets everyone aligned around a direction they can actually commit to."

**Illustration:** Hand-drawn. Represents collaboration and synthesis. Sits above the reactive statement, static.

**Cards (accordion — one open at a time):**

### Vision Sprint — 2–12 weeks
You have fragments of an idea but no shared direction yet. I synthesise research, stakeholder perspectives, and strategic context into tangible artefacts — journey maps, experience flows, IA frameworks, vision decks — that your team can test, validate, and build from.

### Embedded Design Leadership — 3–12 months
A complex initiative needs ongoing orchestration across functions — from concept through delivery. I embed with your team, orchestrate stakeholder alignment, drive design decisions, and keep vision and execution connected.

### Strategic Clarity — Project-based
Your organisation is going through change and needs everyone pointed in the same direction. I design and facilitate the process that turns scattered perspectives into a shared narrative — something that sustains alignment after the engagement ends.

**Differentiator line (below accordion):**
Strategy consultants align on direction — but deliver abstractions. Design agencies execute — but need direction handed to them. I work in the middle: I align through tangible design.

---

## Section 4: Selected Work

**Section label:** Selected work

**Carousel cards — one visual placeholder per card:**

### Card 1
Situation: A music software platform with 900+ products and a CX initiative that six teams were pulling in different directions.
What changed: A shared discovery experience, cross-team alignment, and a foundation that generated €350K in its first quarter.

### Card 2
Situation: A fast-growing point-of-sale product losing merchants in onboarding — with the journey owned by teams who didn't share a map.
What changed: A cross-team vision, an onboarding dashboard shipped, and two quarters of design work ready to execute.

### Card 3
Situation: A complex B2B logistics tool, six product teams, no shared direction — and a tech migration that needed a new interface concept.
What changed: A north-star vision for all six teams. CEO endorsement. Two months to achieve what the client had tried to do for two years.

### Card 4
Situation: A hardware company with no direct customer relationship — dealers owned it. They needed an IoT app craftspeople would actually use.
What changed: 2.5 hours saved daily per professional. Four million tools registered in year one. Three follow-on engagements across 16 countries.

### Final card (CTA)
These are four. There are more.
**Request the portfolio →**

---

## Section 5: Testimonials

**Section label:** What clients say

### 1
"Faced with a tight deadline and complex requirements, Giuseppe distilled vast amounts of information into a clear, coherent structure that users loved. His sharp intellect, calm demeanour and commitment to excellence set him apart."
— Samta Kothari, Lead PM, Sennder

### 2
"Giuseppe's ability to research, synthesise and communicate that synthesis in a clear and inspiring manner is phenomenal. When priorities are changing weekly, this is super helpful."
— Simon Allen, Design Strategy Lead, EOS

### 3
"What sets Giuseppe apart is his thorough discovery and stakeholder management upfront. If you need someone who can architect connected experiences across channels and align diverse stakeholders around a shared vision, Giuseppe delivers."
— Dietrich Pflüger, VP Growth, Native Instruments

### 4
"Giuseppe brings everything together cohesively in an understandable way. His work brought us forward as no one did before."
— Gerrit Elvers, CPO, PRO360 Bosch Power Tools

### 5
"Giuseppe is a great team player and partner in all the stages of product creation. The team and I would not have achieved much without Giuseppe."
— Maria Bykova, Senior MP, SumUp

### 6
"The approach Giuseppe helped establish will continue to influence our customer experience. His approach has rubbed off on those he worked with."
— Simon Cross, CTPO, Native Instruments

### 7
"The research synthesis and ideation workshop you helped lead has seeded a pivot in product strategy. I loved Giuseppe's organisation and ability to respond to swift changes in priorities."
— Samantha Lim, CPO, MoreTo

---

## Section 6: About

**Section label:** About

**Headline:**
I went independent to move faster.

**Body:**
Inside one company, you see one context. Working across logistics, fintech, hardware, music software, and manufacturing — you cross-pollinate. You bring approaches that teams inside one industry rarely encounter. And you can focus on what actually moves things forward, without internal politics slowing you down.

I've been doing this for 15+ years, based in Berlin, working globally.

A few things I hold to:

— Progress happens when people align around something they can see and believe in. That's what I build.
— Trust is built through action, not promises. Quick wins first.
— I don't validate everything. I focus on what actually matters for progress.
— I always recommend a next step. Without one, we stay stuck.

---

## Section 7: Contact

**Headline:**
Let's find out if we're a fit.

**Sub-copy:**
If something here resonates with a challenge you're navigating — or you want to think out loud about a project — I'm happy to have a first conversation with no agenda.

**Form fields:**
- Name
- Company
- What are you trying to figure out?

**Primary CTA:** Send message
**Secondary CTA:** Connect on LinkedIn
